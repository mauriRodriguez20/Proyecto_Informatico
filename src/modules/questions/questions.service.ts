...
async function assertTechnologyIdsExist(technologyIds: string[]): Promise<void> {
  ...
  // Filter out the frontend fallback IDs so they don't trigger validation errors
  const fallbackIds = [
  ...
  const realIds = technologyIds.filter(id => !fallbackIds.includes(id));
