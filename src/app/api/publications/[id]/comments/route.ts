...
export async function GET(
  ...
  try {
    ...
    if (!idValidation.success) {
      const response: ErrorResponse = { error: "El id de la publicacion no es valido." };
    ...
    const result = await listPublicationComments(idValidation.data, validation.data);
    ...
    console.error("[GET /api/publications/:id/comments]", err.message);
...
export async function POST(
  ...
  try {
    ...
    const comment = await createPublicationComment(idValidation.data, userId, validation.data);
    ...
    console.error("[POST /api/publications/:id/comments]", err.message);
    const response: ErrorResponse = { error: "Error interno del servidor." };
    return NextResponse.json(response, { status: 500 });
  }
}
