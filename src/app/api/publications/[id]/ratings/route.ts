...
export async function GET(
  ...
  try {
    ...
    const rating = await getPublicationRatingSummary(idValidation.data);
    ...
    console.error("[GET /api/publications/:id/ratings]", err.message);
...
export async function POST(
  ...
  try {
    ...
    const result = await ratePublication(idValidation.data, userId, validation.data);
    ...
    console.error("[POST /api/publications/:id/ratings]", err.message);
    const response: ErrorResponse = { error: "Error interno del servidor." };
    return NextResponse.json(response, { status: 500 });
  }
}
