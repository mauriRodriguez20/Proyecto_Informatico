...
describe('POST /api/answers/[id]/rate', () => {
  it('retorna 401 si no hay token de autenticacion', async () => {
    ...
    const req = new NextRequest(`http://localhost:3004/api/answers/${ANSWER_ID}/rate`, {
      method: 'POST',
  ...
  it('retorna 400 si el UUID del parametro es invalido', async () => {
    ...
    const req = new NextRequest('http://localhost:3004/api/answers/no-es-uuid/rate', {
      method: 'POST',
  ...
  it('retorna 200 con avgRating y totalRatings cuando la calificacion es exitosa', async () => {
    vi.mocked(withAuth).mockResolvedValue({ userId: 'user-1', errorResponse: null })
    vi.mocked(rateAnswer).mockResolvedValue({
      avgRating: 4.0,
      totalRatings: 5,
    } as any)
    ...
    expect(body.totalRatings).toBe(5)
  })
  ...
  it('retorna 403 si el usuario intenta calificar su propia respuesta', async () => {
  ...
  it('retorna 500 si ocurre un error inesperado', async () => {
    vi.mocked(withAuth).mockResolvedValue({ userId: 'user-1', errorResponse: null })
    vi.mocked(rateAnswer).mockRejectedValue(new Error('DB_CONNECTION_FAILED'))
